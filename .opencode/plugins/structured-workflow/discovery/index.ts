...
async function runScannerSession(
  ...
  try {
    ...
    if (!sessionId) {
      throw new Error(`${scannerName}: could not extract session ID from session.create() response`)
    }
    ...
    const text = extractText(raw, scannerName)
...
function extractText(result: Record<string, unknown>, scannerName: string): string {
  if (Array.isArray(result["parts"])) {
    for (const p of result["parts"] as Array<{ type?: string; text?: string }>) {
  ...
  throw new Error(`${scannerName}: cannot extract text from result: ${JSON.stringify(result).slice(0, 200)}`)
}
...
function buildCombinedReport(scanners: ScannerResult[]): string {
  const lines: string[] = []
...
export async function runDiscoveryFleet(
  ...
  const combinedReport = buildCombinedReport(results)

  return { hasResults, scanners: results, combinedReport }
}
