...
function normalizePrompt(
  ...
  if (normalized !== base) {
    log.warn("Discovery prompt normalized", { scanner: scannerName })
  }
  normalized = normalized.trim()
  if (!normalized) {
    log.warn("Discovery prompt empty", { scanner: scannerName })
    return (
      `You are a discovery scanner named "${scannerName}". ` +
      `The original prompt was empty or invalid. ` +
      `Return a short note stating the prompt was invalid so the main workflow can proceed manually.`
    )
  }
  return normalized
}
...
async function runScannerSession(
  ...
  const normalizedPrompt = normalizePrompt(prompt, scannerName, log)
...
function buildCombinedReport(scanners: ScannerResult[], lowConfidence: boolean): string {
  ...
  if (successCount === 0) {
    lines.push("**Manual fallback required.** All scanners failed.")
    lines.push("Proceed by drafting conventions manually using README/CONTRIBUTING and direct code inspection.")
    lines.push("If desired, re-run discovery after addressing the prompt/template error.")
    lines.push("")
  }
