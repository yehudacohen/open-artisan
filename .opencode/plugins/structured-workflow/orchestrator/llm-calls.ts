  ...
  return async (assessResult, approvedArtifacts) => {
    try {
      if (!assessResult.success) {
        // If assess failed, default to tactical (safe fallback)
 ...
 * The result shape varies by OpenCode version; probe common shapes.
 */
function extractTextFromResult(result: unknown): string {
  if (!result || typeof result !== "object") throw new Error("Empty LLM result")
  const r = result as Record<string, unknown>

  // Shape 1: { parts: [{ type: "text", text: "..." }] }
  if (Array.isArray(r["parts"])) {
    for (const part of r["parts"] as Array<{ type?: string; text?: string }>) {
  ...
  // Shape 4: { output: "..." } (task tool result)
  if (typeof r["output"] === "string") return r["output"]

  throw new Error(`Cannot extract text from LLM result: ${JSON.stringify(result).slice(0, 200)}`)
}
