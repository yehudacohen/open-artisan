...
import { extractSessionId, extractLLMText } from "./type-validation"
import { 
...
export interface IntentComparisonInput {
  ...
  client: any // OpenCode client with session API
  parentModel?: string
}
 ...
 * 
 * @param input - Current intent, prior intent, optional plan path, and client
...
export async function compareIntentsWithLLM(
  ...
  const { currentIntent, priorIntent, priorPlanPath, client, parentModel } = input
  ...
  try {
    if (!client.session) {
      return {
        ...
        explanation: "client.session not available",
        rawResponse: "ERROR: client.session not available",
      }
    }
    ...
    const tempSessionId = extractSessionId(tempSession)
    if (!tempSessionId) {
      return {
        classification: "ERROR",
        explanation: "Could not extract session ID from create response",
        rawResponse: "ERROR: Invalid session creation response",
      }
    }
