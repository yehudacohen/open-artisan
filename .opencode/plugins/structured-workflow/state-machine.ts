...
function buildTable(): TableEntry[] {
  ...
  table.push({ from: ["DISCOVERY", "REVIEW"], event: "self_review_fail", modePredicate: null, to: ["DISCOVERY", "REVIEW"] })
  ...
  for (const phase of STANDARD_PHASES) {
    ...
    table.push({ from: [phase, "REVIEW"], event: "self_review_fail", modePredicate: null, to: [phase, "REVIEW"] })
  ...
  return {
    ...
    validEvents(phase: Phase, phaseState: PhaseState): WorkflowEvent[] {
      return TRANSITION_TABLE
        .filter((e) => e.from[0] === phase && e.from[1] === phaseState)
        .map((e) => e.event)
