...
function buildTaskPrompt(task: TaskNode, progress: TaskProgress): string {
  ...
  if (task.expectedTests.length > 0) {
    ...
    lines.push("All listed tests must pass before calling `request_review`.")
  } else {
  ...
  lines.push("")
  lines.push("When implementation is complete and tests pass, call `request_review`.")
...
export function nextSchedulerDecision(dag: ImplDAG): SchedulerDecision {
  ...
  if (ready.length > 0) {
    ...
    const taskProgress = computeProgress(dag)
    return {
      action: "dispatch",
      task,
      prompt: buildTaskPrompt(task, taskProgress),
      progress: taskProgress,
    }
  }
  ...
  return {
    ...
    message:
      ...
      "but no tasks are currently in-flight. This indicates a DAG state inconsistency. " +
...
/**
 * Marks a task as aborted in the DAG (mutates in place).
 * Also resets its worktree fields (reserved for Layer 4 full parallel).
 */
export function markTaskAborted(dag: ImplDAG, taskId: string): TaskNode | null {
  const task = Array.from(dag.tasks).find((t) => t.id === taskId)
  if (!task) return null
  task.status = "aborted"
